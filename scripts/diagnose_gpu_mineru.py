    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            for i in range(torch.cuda.device_count()):
                ...
                if props.major < 7:
                    print(
                        "      ⚠️  WARNING: MinerU officially requires Volta (7.0+) or later"
                    )
                    print(
                        f"         Your GPU has compute capability {cc} ({arch_name})"
                    )
                    print("         Some features may not work or may fall back to CPU")
                else:
                    print("      ✅ Meets MinerU's Volta (7.0+) requirement")
        else:
...
def check_docling_installation():
    ...
    try:
        ...
        print(f"   Backend: {DoclingParseDocumentBackend.__name__}")
    except ImportError:
...
def test_mineru_gpu():
    ...
    # Check for test file
    test_files = ["test.pdf", "examples/test.pdf", "uploads/test.pdf"]
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            if cc_major < 7:
                ...
                print("\nRecommendations:")
                print(
                    "   1. MinerU may work but some features might be slower or unavailable"
                )
                print("   2. Try backend='pipeline' with device='cuda:0' first")
                print("   3. If you encounter errors, use device='cpu' as fallback")
                print(
                    "   4. Consider upgrading to Ampere (RTX 30xx) or newer for best performance"
                )
                print("\nTo test GPU acceleration:")
                print(
                    "   python -m raganything.parser test.pdf --backend pipeline --device cuda:0"
                )
            else:
                print("✅ Your GPU meets MinerU's requirements")
                print("\nRecommendations:")
                print(
                    "   1. Use backend='pipeline' with device='cuda:0' for best performance"
                )
                print(
                    "   2. Try backend='vlm-transformers' for vision-language model features"
                )
                print("\nTo test GPU acceleration:")
                print(
                    "   python -m raganything.parser test.pdf --backend pipeline --device cuda:0"
