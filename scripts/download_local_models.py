...
def download_model(model_id: str, cache_dir: str) -> Optional[str]:
    ...
    try:
        ...
        print("  2. Try using a HuggingFace mirror: export HF_ENDPOINT=https://hf-mirror.com")
    ...
    try:
        ...
        if model_type == "embedding" or model_type == "multimodal":
            ...
            if hasattr(model[0], 'auto_model'):
                ...
                attn_backend = getattr(getattr(model[0].auto_model, 'config', None), 'attn_implementation', None)
                if attn_backend:
        ...
        if expected_vram > 0:
            if memory_used > expected_vram * 1.2:
                print(f"⚠ Warning: Memory usage ({memory_used:.2f} GB) exceeds expected ({expected_vram:.2f} GB)")
            elif memory_used < expected_vram * 0.8:
                print(f"⚠ Warning: Memory usage ({memory_used:.2f} GB) is lower than expected ({expected_vram:.2f} GB)")
