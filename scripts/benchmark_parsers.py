...
def print_summary(results: List[Dict[str, Any]]):
    ...
    if successful:
        ...
        for i, result in enumerate(successful, 1):
            ...
            print(f"{i:<6} {result['parser']:<30} {result['time']:>8.2f}s   "
    ...
    if successful:
        ...
        if gpu_results and cpu_results:
            ...
            if speedup < 1.5:
                print(f"   ⚠️  GPU speedup is minimal (<1.5x)")
                print(f"      This may indicate:")
                print(f"      - GPU architecture limitations (Pascal vs Volta)")
                print(f"      - Small document size (GPU overhead dominates)")
                print(f"      - CPU-bound operations in the pipeline")
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark MinerU and Docling parsers"
    )
    parser.add_argument(
        "file",
