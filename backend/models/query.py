...
class QueryRequest(BaseModel):
    ...
    mode: str = Field(
        default="hybrid", description="Query mode: naive, local, global, or hybrid"
    )
    top_k: Optional[int] = Field(
        default=None, description="Number of top results to retrieve", ge=1, le=100
    )
    max_tokens: Optional[int] = Field(
        default=None, description="Maximum tokens in response", ge=1
    )
...
class MultimodalContent(BaseModel):
    ...
    table_data: Optional[str] = Field(
        default=None, description="Table data (CSV or markdown)"
    )
    ...
    equation_caption: Optional[str] = Field(
        default=None, description="Equation caption"
    )
...
class MultimodalQueryRequest(BaseModel):
    ...
    multimodal_content: Optional[List[MultimodalContent]] = Field(
        default=None, description="Optional multimodal content items"
    )
    mode: str = Field(
        default="hybrid", description="Query mode: naive, local, global, or hybrid"
    )
