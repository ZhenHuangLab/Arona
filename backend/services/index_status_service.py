...
class IndexStatusService:
    ...
    def get_status(self, file_path: str) -> Optional[IndexStatus]:
        ...
        try:
            cursor = conn.execute(
                "SELECT * FROM index_status WHERE file_path = ?",
                (file_path,)
            )
            row = cursor.fetchone()
    ...
    def list_all_status(self) -> List[IndexStatus]:
        ...
        conn = self._get_connection()
        try:
            cursor = conn.execute("SELECT * FROM index_status ORDER BY last_modified DESC")
    ...
    def upsert_status(self, status: IndexStatus) -> None:
        ...
        try:
            ...
            conn.commit()
            logger.debug(f"Upserted status for {status.file_path}: {status.status.value}")
        except sqlite3.Error as e:
            logger.error(f"Failed to upsert status for {status.file_path}: {e}", exc_info=True)
            raise
        finally:
            conn.close()
    ...
    def update_status_field(
        ...
        try:
            ...
            logger.error(f"Failed to update {field} for {file_path}: {e}", exc_info=True)
            raise
        finally:
            conn.close()
