...
from backend.services.file_scanner import scan_upload_directory
...
class BackgroundIndexer:
    ...
    async def process_pending_files(self) -> None:
        ...
        try:
            ...
            pending_files = [
                status for status in all_statuses if status.status == StatusEnum.PENDING
            ]
    ...
    async def _process_single_file(self, status: IndexStatus) -> None:
        ...
        try:
            ...
            self.index_status_service.update_status_field(
                status.file_path, "status", StatusEnum.PROCESSING
            )
        ...
        try:
            while True:
                try:
                    ...
                    logger.error(
                        f"Error in background indexer iteration: {e}", exc_info=True
                    )
