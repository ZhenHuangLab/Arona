...
class QueryMixin:
    ...
    async def aquery(self, query: str, mode: str = "mix", **kwargs) -> str:
        ...
        # Apply rerank defaults before building query parameters
    ...
    async def aquery_with_multimodal(
        ...
        **kwargs,
        ...
        # Apply defaults such as rerank toggles before executing
        kwargs = self._apply_query_defaults(kwargs)
    ...
    def _apply_query_defaults(self, kwargs):
        enable_rerank = kwargs.get("enable_rerank")
        rerank_ready = self._should_enable_rerank()

        if enable_rerank is None:
            kwargs["enable_rerank"] = rerank_ready
        elif enable_rerank and not rerank_ready:
            # Reason: LightRAG logs noisy warnings when rerank is requested without a model.
            kwargs["enable_rerank"] = False
            self.logger.warning(
                "FlagEmbedding reranker not configured; disabling rerank for this query."
            )
        return kwargs

    def _should_enable_rerank(self) -> bool:
        config_flag = getattr(self.config, "enable_rerank", False)
        return bool(config_flag and getattr(self, "rerank_model_func", None))
