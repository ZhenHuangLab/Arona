...
class RAGAnything(QueryMixin, ProcessorMixin, BatchMixin):
    ...
    """Embedding function used by LightRAG."""

    rerank_model_func: Optional[Callable] = field(default=None)
    """Optional reranker callable passed to LightRAG."""
    ...
    _ollama_client: Optional[Any] = field(default=None, init=False, repr=False)
    """Cached Ollama client used when default bindings are enabled."""

    _reranker_wrapper: Optional[Any] = field(default=None, init=False, repr=False)
    """Cached FlagEmbedding wrapper when reranking is enabled."""
    ...
    def _setup_default_integrations(self) -> None:
        """Provision default model bindings from configuration when missing."""

        self._setup_default_ollama_bindings()
        self._setup_default_reranker()

        # Propagate externally provided reranker into LightRAG init kwargs.
        # Reason: When backend passes rerank_model_func, we must forward it to LightRAG,
        # otherwise LightRAG will warn that rerank is enabled without a configured model.
        if self.rerank_model_func is not None:
            self.lightrag_kwargs.setdefault("rerank_model_func", self.rerank_model_func)

    def _setup_default_ollama_bindings(self) -> None:
        if not self.config:
            return
        ...
        if self.llm_model_func is None:
            self.llm_model_func = self._create_ollama_llm_func(client)
            self.logger.info(
                "Configured default Ollama LLM model: %s",
                self.config.ollama_llm_model,
            )

        if self.embedding_func is None:
            self.embedding_func = self._create_ollama_embedding_func(client)
            self.logger.info(
                "Configured default Ollama embedding model: %s",
                self.config.ollama_embed_model,
            )

    def _create_ollama_llm_func(self, client):  # type: ignore[override]
        async def _call_llm(
            prompt,
            system_prompt=None,
            ...
            **kwargs,
            ...
            options_keys = {"temperature", "top_p", "seed", "num_ctx"}
            options = {
                key: kwargs[key]
            ...
            try:
                ...
                response = await asyncio.to_thread(
                    ...
                    stream=False,  # Force non-streaming for sync client
            ...
            if stream:
                # Reason: LightRAG expects AsyncIterator when streaming is requested.
    ...
    def _create_ollama_embedding_func(self, client):
    ...
    def _setup_default_reranker(self) -> None:
        if not self.config or not getattr(self.config, "enable_rerank", False):
            return
        ...
        try:
            ...
            wrapper = FlagEmbeddingReranker(
                self.config.reranker_model_path,
                use_fp16=self.config.reranker_use_fp16,
                batch_size=self.config.reranker_batch_size,
            )
        except Exception as exc:  # pylint: disable=broad-except
            self.logger.warning("Disabling reranker: %s", exc, exc_info=False)
    ...
    def close(self):
        ...
        try:
            ...
            try:
                ...
                print(f"Warning: Failed to finalize RAGAnything storages: {exc}")
            finally:
                loop.close()
        else:
            # Loop is alive; schedule cleanup without blocking shutdown
    ...
    def get_config_info(self) -> Dict[str, Any]:
        ...
        config_info["ollama"] = {
            "base_url": getattr(self.config, "ollama_base_url", ""),
            "llm_model": getattr(self.config, "ollama_llm_model", ""),
            "embed_model": getattr(self.config, "ollama_embed_model", ""),
            "embedding_dim": getattr(self.config, "ollama_embedding_dim", None),
        }

        config_info["reranker"] = {
            "enabled": getattr(self.config, "enable_rerank", False)
            and self.rerank_model_func is not None,
            "model_path": getattr(self.config, "reranker_model_path", ""),
            "use_fp16": getattr(self.config, "reranker_use_fp16", False),
        }
